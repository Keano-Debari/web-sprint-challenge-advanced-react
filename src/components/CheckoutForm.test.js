...
test("form shows success message on submit with form details", () => {
    ...
    userEvent.type(lastName, 'Universe');
    userEvent.type(address, '1234 Hill Way');
    userEvent.type(city, 'Beach City');
    userEvent.type(state, 'CA');
    userEvent.type(zip, '57343');
    userEvent.click(button);
    
    
    const orderProcessedMessage = screen.getByText(/green friends/i);
    const customerDetails = screen.getByText(/Steven/i);
